...
export default function CabinetPage() {
  ...
  const role    = profile?.role === 'vendor' ? 'Виконавець' : 'Клієнт';
          ...
          <div className="cab-section">
            <div className="cab-tabs">
              <button className={`vp-tab ${tab === 'requests' ? 'on' : ''}`} onClick={() => setTab('requests')}>
                Мої запити {requests.length > 0 && <span className="cab-count">{requests.length}</span>}
              </button>
                    ...
                    return (
                      <div key={r.id} className="cab-req">
                        <div className="cab-req-head">
                          <div>
                            <div className="cab-req-vendor">{r.vendor_name}</div>
        ...
        <aside className="cab-side">
          <div className="cab-card">
            ...
            <div className="cab-rows">
              ...
              <div className="cab-row"><span className="mono dim">ЗАПИТИ</span><span style={{ color: 'var(--gold)' }}>{requests.length}</span></div>
            </div>
            {profile?.role === 'vendor' && (
